...
def configure_interface():
    ...
    uploaded_file = st.file_uploader("Escolher um arquivo", type=["png", "jpg", "jpeg"])
    ...
    if uploaded_file is not None:
        ...
        if blob_url:
            st.write(f"Arquivo {fileName} enviado com sucesso ao Azure Blob Storage")
            credit_card_info = analize_credit_card(blob_url)
            show_image_and_validation(blob_url, credit_card_info)
        else:
            st.write(f"Erro ao enviar o aquivo {fileName} ao Azure Blob Storage")
...
def show_image_and_validation(blob_url, credit_card_info):
    ...
    if credit_card_info and credit_card_info['card_name']:
        ...
        st.markdown(f"<h1 style='color: red;'> Cartão Inválido</h1>", unsafe_allow_html=True)
        st.write("Este não é um cartçao de crédito válido.")
